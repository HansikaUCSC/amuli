...
class Home extends CI_Controller
{
	...
	public function search_user(){
		...
		if ($this->form_validation->run() == False) {
	        ...
	        $query = $this->home_model->search_user($name,$location,$skill);
	        $data['user'] = null;
	        if ($query) {
	        	$data['user'] = $query;
	        	$this->load->view('nav');
	...
	public function view_profile($id){
		$query = $this->home_model->view_profile_model($id);
		$query1 = $this->home_model->load_skill($id);
		if ($query) {
			$data['profile'] = $query;
			$data['skill'] = $query1;
			$this->load->view('nav');
	...
	public function feedback($id){
		$query = $this->home_model->feedback($id); 
